...
from projects.llava_sam2.models.preprocess.qwen_preprocess import QwenPrepocessor
...
class VideoLLaVASAMModel_Qwen(VideoLLaVASAMModel):
    ...
    def preparing_for_generation(self, tokenizer, max_new_tokens=2048, torch_dtype=torch.bfloat16):
        ...
        self.max_dynamic_patch = 12
        self.downsample_ratio = 0.5
        self.min_pixels = 16*16*28*28
        self.max_pixles = 16*16*28*28
        self.use_thumbnail = True
        ...
        self.patch_token = 1
        self.transformer = None
        self.preprocessor = QwenPrepocessor("pretrained/Qwen2.5-VL-3B-Instruct", self.min_pixels, self.max_pixles)
    ...
    def predict_forward(
        ...
        if image is None and video is None and '<image>' not in past_text:
            ...
            input_dict['g_pixel_values'] = g_pixel_values.cuda()
        ...
        generate_output = self.mllm.model.generate(
            **mm_inputs,
            generation_config=self.gen_config,
            streamer=None,
            bos_token_id=self.tokenizer.bos_token_id,
