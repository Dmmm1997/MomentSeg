...
def main():
    ...
    file = open(os.path.join(args.work_dir, f'GCG_{split}.txt'), 'a+', encoding='utf8')
    # # -------------------------------#
    ...
    print(f"Mean IoU (mIoU) across all images: {miou:.3f}")
    file.write(f"Mean IoU (mIoU) across all images: {miou:.3f}\n")

    # # -------------------------------#
    # 4. Evaluate Recall
    recall = evaluate_recall_with_mapping(coco_gt, coco_cap_gt, all_images_ids, pred_save_path, cap_pred_save_path,
