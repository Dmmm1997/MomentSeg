...
def parse_args():
    ...
    parser.add_argument('--min_pixels', type=int, default=8*8*28*28)
...
def main():
    ...
    for idx in tqdm.tqdm(per_rank_ids):
        ...
        if 'prediction_masks' not in pred_dict.keys() or pred_dict['prediction_masks'] is None or len(pred_dict['prediction_masks']) == 0:
            ...
            prediction['prediction_masks'] = torch.stack(
                [torch.from_numpy(x) for x in pred_dict['prediction_masks']], dim=0
            )[:, 0]
