...
if __name__ == '__main__':
    ...
    for item in tqdm.tqdm(dataloader):
        with torch.no_grad():
            ...
            total_frames_processed += item['length']  # 累加处理的帧数
            # -------------------------
    ...
    if total_inference_time > 0:
        average_fps = total_frames_processed / total_inference_time
        print(f'[Rank {rank}] : Processed {total_frames_processed} frames in {total_inference_time:.2f} seconds.')
        print(f'[Rank {rank}] : Average Inference Speed: {average_fps:.2f} FPS')
    else:
        print(f'[Rank {rank}] : No frames processed or time was zero.')
    # ---------------------------
