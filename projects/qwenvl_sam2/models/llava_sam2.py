...
class VideoLLaVASAMModel(LisaModel):
    ...
    def state_dict(self, *args, **kwargs):
        ...
        to_return.update(
            {k: v
             for k, v in state_dict.items() if 'find_hidden_fcs.' in k}
        )
        to_return.update(
            {k: v
             for k, v in state_dict.items() if 'img_hidden_fcs.' in k}
        )
        # to_return.update(
        #     {k: v
        #      for k, v in state_dict.items() if 'seg_attn.' in k}
        # )
        to_return.update(
            {k: v
...
class VideoLLaVASAMModel_Qwen(VideoLLaVASAMModel):
    def __init__(self,
                 mllm,
                 tokenizer,
                 grounding_encoder,
        ...
        super(VideoLLaVASAMModel_Qwen, self).__init__(
            mllm=mllm,
            tokenizer=tokenizer,
            grounding_encoder=grounding_encoder,
            loss_mask=loss_mask,
        ...
        self.bs = bs
        if find_key_frames and "[FIND]" in special_tokens:
            self.find_token_idx = self.tokenizer("[FIND]", add_special_tokens=False).input_ids[0]
        self.video_token_idx = self.tokenizer("<|video_pad|>", add_special_tokens=False).input_ids[0]
        self.find_key_frames = find_key_frames
        self.image_token_idx = self.tokenizer("<|image_pad|>", add_special_tokens=False).input_ids[0]
        self.perframe_seg = perframe_seg
    ...
    def _compute_find_similarity(self, find_hidden_states, video_token_mask, find_token_mask, video_key_frames, video_grid_thw):
        find_logits_list = []
        video_key_frame_list = []
        for b in range(find_hidden_states.size(0)):
            if video_token_mask[b].sum()==0 or find_token_mask[b].sum()==0: # if do not have the <video_pad> pass this sample
                continue
            video_key_frame = video_key_frames[b]
            t, h, w = video_grid_thw[b][0]
            T = video_key_frame.shape[1]
            find_tokens = find_hidden_states[b][find_token_mask[b]]
            video_tokens = find_hidden_states[b][video_token_mask[b]]
            video_tokens = video_tokens.reshape(t, -1, video_tokens.shape[-1])
            # video_tokens_pooled = F.adaptive_avg_pool1d(video_tokens.transpose(0,1), output_size=T).transpose(0,1)
            video_tokens_pooled = video_tokens.mean(1) # t, C
            video_tokens_pooled = video_tokens_pooled.unsqueeze(0).transpose(1,2) # 1, C, t
            video_tokens_pooled = F.interpolate(video_tokens_pooled, size=T, mode='linear', align_corners=True).squeeze().transpose(0,1) # T,C
            similarity = torch.nn.functional.cosine_similarity(find_tokens.unsqueeze(1), video_tokens_pooled.unsqueeze(0), dim=-1, eps=1e-8).reshape(-1)
            find_logits = similarity / 0.07
            find_logits_list.append(find_logits)
            video_key_frame_list.append(video_key_frame.reshape(-1))
        find_logits = torch.cat(find_logits_list)
        find_labels = torch.cat(video_key_frame_list).to(find_logits.device)
        assert len(find_logits)==len(find_labels)
        return find_logits, find_labels
    ...
    def genetate_video_perframe_pred_embeddings(self, pred_embeddings_list, frames_per_batch, image_features=None, image_size=None):
        if len(pred_embeddings_list) == len(frames_per_batch):
            success = True
        else:
            success = False
            print("len(pred_embeddings_list):{} is not equal to len(frames_per_batch):{} !!!".format(len(pred_embeddings_list), len(frames_per_batch)))
        pred_embeddings_list_video = []
        for image_feat, pred_embedding_batch, frame_nums, frame_size in zip(image_features, pred_embeddings_list, frames_per_batch, image_size):
            image_feature = image_feat.reshape(frame_nums, frame_size[0][1]//2, frame_size[0][2]//2, 256).permute(0, 3, 1, 2)
            pred_embedding_batch_ = pred_embedding_batch.repeat(frame_nums, 1)
            pred_embeddings_list_video += [item for item in self.seg_attn(pred_embedding_batch_, image_feature.repeat_interleave(len(pred_embedding_batch), dim=0)).reshape(frame_nums, len(pred_embedding_batch), 256)]
            # 这种方式得到的结果是[frame1-seg1,frame1-seg2,....]
        return pred_embeddings_list_video, success

    def prepare_finding_video_target(self, video_token_mask, video_grid_thw, video_key_frames):
        # 需要补全video_grid_thw和video_key_frames
        new_video_grid_thw, new_video_key_frames = [],[]
        this_index = 0
        for index in range(len(video_token_mask)):
            if video_token_mask[index].sum()==0:
                new_video_grid_thw.append(None)
                new_video_key_frames.append(None)
            else:
                new_video_grid_thw.append(video_grid_thw[this_index])
                new_video_key_frames.append(video_key_frames[this_index])
                this_index+=1
        return new_video_grid_thw, new_video_key_frames

    def forward(self, data, data_samples=None, mode='loss'):
        g_pixel_values = data.pop('g_pixel_values', None)
        gt_masks = data.pop('masks', None)
        video_key_frames = data.pop('video_key_frames', None) # B, 5, 50
        frames_per_batch = data.pop('frames_per_batch', None)
        ...
        seg_token_mask = input_ids == self.seg_token_idx
        video_token_mask = input_ids == self.video_token_idx
        image_token_mask = input_ids == self.image_token_idx
        
        hidden_states = output.hidden_states
        seg_hidden_states = self.text_hidden_fcs(hidden_states[-1])
        find_hidden_states = self.find_hidden_fcs(hidden_states[-1])
        image_hidden_states = self.img_hidden_fcs(hidden_states[-1])

        image_embeddings = [image_hidden_states[i][image_token_mask[i]] for i in range(len(input_ids))]
        
        if self.find_key_frames and video_key_frames is not None and (input_ids == self.find_token_idx).sum()>0 and video_grid_thw is not None:
            find_loss = 0.0
            new_video_grid_thw, new_video_key_frames = self.prepare_finding_video_target(video_token_mask, video_grid_thw, video_key_frames)
            if (input_ids == self.find_token_idx).sum()>0:
                find_token_mask = input_ids == self.find_token_idx
                logits, labels = self._compute_find_similarity(find_hidden_states, video_token_mask, find_token_mask, new_video_key_frames, new_video_grid_thw)
                weights = torch.ones_like(labels, dtype=torch.float)
                weights[labels == 1] = 2.0 # increase weight for negatives
                find_loss += F.binary_cross_entropy_with_logits(logits, labels.float(), weight=weights, reduction='mean') * 0.5
        else:
            find_token_mask = torch.zeros_like(input_ids).bool()
            find_token_mask[:,0:5]=True
            video_token_mask = torch.zeros_like(input_ids).bool()
            video_token_mask[:,0:2*9]=True
            video_grid_thw = [[(2, 3, 3)]] * len(input_ids)
            video_key_frames = [torch.zeros((5, 4), dtype=torch.long, device=input_ids.device)] * len(input_ids)
            logits, labels = self._compute_find_similarity(find_hidden_states, video_token_mask, find_token_mask, video_key_frames, video_grid_thw)
            labels = torch.zeros_like(logits)
            find_loss = F.binary_cross_entropy_with_logits(logits, labels.float(), reduction='mean') * 0.0

        if torch.isnan(find_loss):
            print("NaN detected!")
            print("logits:", logits)
            print("labels:", labels)

            
        _zero = seg_hidden_states.mean() * 0.0
        if seg_valid:
            pred_embeddings = seg_hidden_states[seg_token_mask] + _zero
        else:
            pred_embeddings = seg_hidden_states[:, :5].flatten(0, 1) + _zero
        ...
        if self.perframe_seg:
            if image_grid_thw is None:
                image_grid_thw = [None for i in range(len(frames_per_batch))]
            image_grid_thw_new = []
            image_embeddings_new = []
            frame_per_batch_new = []
            pred_embeddings_list_new = []
            for ind, img_grid_thw_ in enumerate(image_grid_thw):
                if img_grid_thw_ is None:
                    image_grid_thw_new.append(image_hidden_states[ind,:25])
                    image_grid_thw_new.append(torch.tensor((1,10,10)))
                    frame_per_batch_new.append(1)
                    pred_embeddings_list_new.append(image_hidden_states[ind,25])
                else:
                    image_embeddings_new.append(image_embeddings[ind])
                    image_grid_thw_new.append(image_grid_thw[ind])
                    frame_per_batch_new.append(frames_per_batch[ind])
                    pred_embeddings_list_new.append(pred_embeddings_list[ind])

            pred_embeddings_list_video, success = self.genetate_video_perframe_pred_embeddings(
                pred_embeddings_list_new, frame_per_batch_new, image_embeddings_new, image_grid_thw_new)
        else:
            pred_embeddings_list_video, success = self.genetate_video_pred_embeddings(
                pred_embeddings_list, frames_per_batch)
