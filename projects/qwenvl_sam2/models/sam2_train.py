...
class SAM2TrainRunner(BaseModule):
    def __init__(
        ...
        if "sam2.1" in cfg_path:
            import third_parts.sam2_1
        else:
            import third_parts.sam2 # important for finding the config file
        ...
        if "sam2.1" in cfg_path:
            hydra_overrides = [
                ...
                "++model._target_=projects.qwenvl_sam2.models.extension.sam2_1_base.SAM2_1Base",
            ]
        else:
            hydra_overrides = [
                ## Extension: LLM prompt
                "++model._target_=projects.qwenvl_sam2.models.extension.sam2_base.SAM2Base",
            ]
...
class SAM2TrainRunnerv2(SAM2TrainRunner):
    ...
    def language_embd_inference(self, inference_state, language_embd, frame_indexes):
        num_frame = len(language_embd)
        num_obj = len(language_embd[0])
        mask_out = []
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
