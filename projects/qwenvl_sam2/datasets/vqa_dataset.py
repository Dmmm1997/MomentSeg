...
class InfinityMMDataset(Dataset):
    ...
    def prepare_data(self, index):
        ...
        if len(token_dict["input_ids"]) >= self.max_length:
            print(f'VQA: Skip this sample due to its long context')
...
class LLaVADataset(Dataset):
    ...
    def prepare_data(self, index):
        ...
        if data_dict.get('image', None) is not None:
            ...
            # out_data_dict.update(token_dict)
            # out_data_dict['pixel_values'] = None
            # out_data_dict['image_grid_thw'] = None
            # out_data_dict['pixel_values'] = torch.zeros(200, 28*28*2)
            # out_data_dict['image_grid_thw'] = torch.tensor([1, 10, 10])
            # print(f'LLAVA_VQA: Skip this sample due to no images existing')
            return self.__getitem__(random.randint(0, len(self)))
