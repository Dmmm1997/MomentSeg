...
def video_lisa_collate_fn(instances: Sequence[Dict],
    ...
    has_video_key_frame = any(inst.get('video_key_frames') is not None for inst in instances)
    ...
    for example in instances:
        ...
        if has_image and 'pixel_values' in example.keys():
        ...
        if has_video and 'pixel_values_videos' in example.keys():
        ...
        if has_video_key_frame:
            if 'video_key_frames' in example.keys():
                _video_key_frames = np.stack(example['video_key_frames'], axis=0)
                video_key_frames.append(torch.from_numpy(_video_key_frames))
    ...
    if has_video:
        # if all(x.shape == pixel_values_videos[0].shape for x in pixel_values_videos):
    ...
    if has_video_key_frame:
        data_dict['video_key_frames'] = video_key_frames
