...
class ReferSegmDataset(RefCocoDataset):
    ...
    def __init__(self,
        ...
        print("Image RES dataset, include {} items.".format(self.real_len()))
    ...
    def modality_length(self):
        return [100 for _ in range(self.real_len()*self.repeat)]
    ...
    def prepare_data(self, index):
        ...
        if data_dict.get('image', None) is not None:
            ...
            print(f'RefCOCO: Skip this sample due to no images existing')
            return self.__getitem__(random.randint(0, len(self)))

        if len(token_dict["input_ids"]) >= self.max_length:
            print(f'RefCOCO: Skip this sample due to its long context')
            return self.__getitem__(random.randint(0, len(self)))
