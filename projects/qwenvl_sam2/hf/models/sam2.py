...
class SAM2(nn.Module):
    ...
    def language_embd_inference(self, inference_state, language_embd):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            for out_frame_idx, out_obj_ids, out_mask_logits in self.sam2_model.propagate_in_video(inference_state, start_frame_idx=0):
                mask_out.append(out_mask_logits)
            mask_out = torch.cat(mask_out, dim=0)
        return mask_out

    def language_embd_inference_withbap(self, inference_state, language_embd, frame_indexes, center_index=None):
        num_frame = len(language_embd)
        num_obj = len(language_embd[0])
        ...
        if len(right_direction_index)>0: # no need for reversing
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                right_mask_out = []
                inference_state["language_embd"] = _language_embd
                for out_frame_idx, out_obj_ids, out_mask_logits in self.sam2_model.bidirectional_adaptive_propagate_in_video(inference_state, start_frame_idx=center_frame_index, memory_clean=True):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            left_mask_out = []
            inference_state["language_embd"] = _language_embd
            for out_frame_idx, out_obj_ids, out_mask_logits in self.sam2_model.bidirectional_adaptive_propagate_in_video(inference_state, start_frame_idx=center_frame_index, reverse=True, memory_clean=True):
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def _run_single_frame_inference(
        ...
        object_scores = torch.sigmoid(current_out["object_score_logits"])
    ...
    def _consolidate_temp_output_across_obj(
        ...
        consolidated_out["object_scores"] = out["object_scores"].mean()
    ...
    def propagate_in_video(
        ...
        language_embd = inference_state.get("language_embd", None)
        output_dict = inference_state["output_dict"]
        consolidated_frame_inds = inference_state["consolidated_frame_inds"]
        obj_ids = inference_state["obj_ids"]
        num_frames = inference_state["num_frames"]
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            if frame_idx in consolidated_frame_inds["cond_frame_outputs"]:
                ...
                pred_mask_score = current_out["object_scores"]
                tracking_score = torch.prod(torch.stack(tracking_scores)) if len(tracking_scores)>0 else 2
                if clear_non_cond_mem or (memory_clean and tracking_score<pred_mask_score-0.1):
                    # clear non-conditioning memory of the surrounding frames
                    self._clear_non_cond_mem_around_input(inference_state, frame_idx)
                ...
                current_out, pred_masks = self._run_single_frame_inference(
                    ...
                    reverse=reverse,
                    run_mem_encoder=True,
                    # language_embd=language_embd, #TODO BAP added
                )
                output_dict[storage_key][frame_idx] = current_out
                tracking_scores.append(current_out["object_scores"].mean())
    ...
    @torch.inference_mode()
    def bidirectional_adaptive_propagate_in_video(
        self,
        inference_state,
        start_frame_idx=None,
        max_frame_num_to_track=None,
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            if frame_idx in consolidated_frame_inds["cond_frame_outputs"]:
                ...
                pred_mask_score = current_out["object_scores"]
                tracking_score = torch.prod(torch.stack(tracking_scores)) if len(tracking_scores)>0 else 2
                if clear_non_cond_mem or (memory_clean and tracking_score<pred_mask_score*0.9):
                    # clear non-conditioning memory of the surrounding frames
                    self._clear_non_cond_mem_around_input(inference_state, frame_idx)
                    tracking_scores = []
            elif frame_idx in consolidated_frame_inds["non_cond_frame_outputs"]:
...
def get_connected_components(mask):
    ...
    os.system("wget -c https://github.com/facebookresearch/sam2/blob/main/sam2/csrc/connected_components.cu")
